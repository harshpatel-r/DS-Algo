...
the number of candies in the box that Bob must exchange. If there are multiple answers, you may return any one of them. It is 
...
var fairCandySwap = function(aliceSizes, bobSizes) {
    let aliceCandySum = aliceSizes.reduce((accum, num) => accum + num);
    let bobCandySum = bobSizes.reduce((accum, num) => accum + num);

    let bobSet = new Set(bobSizes);

    for (let i = 0; i < aliceSizes.length; i++) {
        let targetBobSize = (bobCandySum - aliceCandySum + 2 * aliceSizes[i]) / 2;
        if (bobSet.has(targetBobSize)) {
